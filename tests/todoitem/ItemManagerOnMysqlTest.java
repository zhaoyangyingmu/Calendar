...
public class ItemManagerOnMysqlTest {
    ...
    public void setCompleted() throws Exception {
        ...
        ArrayList<Item> items = new ArrayList<Item>() {
            {
                add(new MeetingItem(new TimeStamp(2018, 5, 26, 0, 0),
                        new TimeStamp(2019, 6, 1, 23, 50),
                        "进行中", "", ""));

                add(new MeetingItem(new TimeStamp(2018, 5, 20, 0, 0),
                        new TimeStamp(2018, 5, 24, 23, 50),
                        "已过期", "", ""));

                add(new AppointmentItem(new TimeStamp(2019, 6, 2, 0, 0),
                        new TimeStamp(2019, 6, 2, 23, 59),
                        "未开始", "", ""));

                add(new AppointmentItem(new TimeStamp(2018, 2, 20, 0, 1),
        ...
        for (Item item : items) {
            Mysql.addSchedule(item.getAttrs());
        }
        ...
        for (Item item : items) {
            if (item.getStatus() == Const.OVERDUE || item.getStatus() == Const.BEFORE_BEGINNING
                ...
                Item item1 = new OtherItem(new TimeStamp(2018, 5, 20, 6, 0),
                        new TimeStamp(2019, 5, 20, 0, 0), "已完成");
                Item item2 = new OtherItem(new TimeStamp(2018, 5, 20, 8, 0),
                        new TimeStamp(2020, 2, 10, 0, 0), "已完成");
